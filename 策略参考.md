...
| V3-B 风险平价(20d) | `strategy_b.py::backtest_b` → 统一`backtest()` | 4 桶等权 HRP + nonferr 75d + gold 75d + sp500 120d + hs300 30d + Gold dip + HS300 AND抄底 + 危机波动率控制 | 8 (无 bond_10y, 含 wti/copper) | CAGR 8.94%，四重趋势+危机波动率控制 |
| V3c 多元 | `backtest.py::backtest_iv` → 统一`backtest()` | 逆波动率 60d + nonferr 75d + **SP500趋势 75d** + HS300 AND抄底 | 8 (含 wti, copper, 不含 bond_10y) | CAGR 8.86%, Sharpe 1.16, MDD -7.26%, Calmar 1.22 |
...
V3-B RP 和 V3c 去掉了 bond_10y：CAGR +1.43pp，Sharpe 仅 -0.02。V3c 额外增加了 SP500 趋势过滤(75d) 降低权益下行风险。
...
| `LEVERAGE_FACTORS["bond_10y"]` | 3.0 | V4 T.CFFEX 10Y 国债期货正常杠杆；bond_30y 破 SMA120 时动态降至 1.5x |

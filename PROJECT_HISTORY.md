...
特征：商品 27%，CAGR 7.24%，回撤最深 -8.59%。

### 4.4 三套核心指标对比（100% RP 档，2015-01 ~ 2025-12）

| 方案 | CAGR | 波动 | 回撤 | Sharpe | Calmar |
|---|---|---|---|---|---|
| V3c 多元 | **7.45%** | 4.16% | -6.71% | **1.26** | 1.11 |
| V3b 平衡 | 7.12% | 4.03% | -6.99% | 1.22 | 1.02 |
| V3d 商品偏重 | 7.24% | 4.63% | -8.59% | 1.09 | 0.84 |
...
纯固定权重（含 20% short_bond）+ 阈值再平衡，10 资产。无择时层。
...
特征：11 年 CAGR 6.83%，MDD -8.16%，Sharpe 1.14。short_bond 降低久期但牺牲了收益。

> 2026-05-25 精简：原版 V3-A 含三层风控（趋势过滤+回撤止损）。诊断发现回撤止损 11 年 0 触发（阈值 -8% > 实际 max DD -6.74%），趋势过滤归一化逻辑导致追涨杀跌。砍掉全部择时层后降格为纯固定权重策略。

### 4.6 V3-B 分层风险平价（★★★ 60d / ★★★ 120d）
...
| 文件 | 格式 | 用途 |
|---|---|---|
| `output/nv_curves.csv` | 宽表 CSV | 18 条净值曲线，画图直接用 |
| `output/weights.csv` | 表格 CSV | 六策略权重 |

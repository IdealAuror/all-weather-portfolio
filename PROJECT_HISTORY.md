...
| 2026-05-26 | HS300 抄底收编：沪深300回撤>35%翻倍增持（从信用债提取）。仅2015股灾(-44%)和2021-22地产危机(-34%)触发，普通熊市(2018 -29%, 2023 -20%)不触发。V3-B CAGR 8.47→9.24%（+0.77%），Sharpe 1.73→1.80，MDD不变；保守增强 CAGR 7.03→7.61%（+0.58%），Sharpe 1.99→2.05。A股真正大熊市跌幅中位数~32%，35%阈值恰好卡在"普通熊市"和"史诗级崩盘"之间。低阈值(25%)会恶化MDD和Sharpe，因为抄在半山腰。 |
...
**22. HS300 抄底需要高阈值（35%），本质是"股灾保险"而非"逢低买入"。** A股真正熊市跌幅中位数~32%（2015 -44%，2018 -29%，2021-22 -34%，2023 -20%）。阈值过低（10-25%）会在半山腰接刀，导致 MDD 恶化和 Sharpe 下降。35% 阈值恰好只触发两次史诗级崩盘（2015、2021-22），不影响普通熊市（2018、2023）。CAGR 从 8.47→9.24%（+0.77%），MDD 和 Sharpe 同时改善。这与 Gold 抄底逻辑一致：对趋势性资产（权益）需要更高的安全边际。两个抄底机制共享同一资金来源（credit），构成多层防御体系——Gold 15% 抓周期、HS300 35% 接股灾。

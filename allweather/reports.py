...
def print_summary_recommendation(perf_results=None):
    ...
    cards = [
        ...
        ("V3c 多元", "★★★", "多元全天候", "8资产逆波动率 60d + nonferr(75d) + HS300 AND抄底",
         [f"+ 中位回报 CAGR {_pv('V3c 多元','cagr','pct','%')}，不含 bond_10y 结构更轻",
          f"+ 多元资产组合(+铜+原油)覆盖通胀敞口更全面",
          "+ 无杠杆 + 简单逆波动率，实盘可执行性强",
          f"- 回撤({_pv('V3c 多元','mdd','pct','%')})接近风险平价水平",
          "- 不含10Y国债，久期敞口略低"],

...
PORTFOLIO_TAGS = {
    ...
    "CI011001 全天候":     {"stars": "★★★★★", "label": "CI011001 复刻 — ERC风险平价60d + 双向目标波动率5% + 动态杠杆 + 月频调仓"},
}

# ============================================================================
# CI011001 策略常量 — 对标国泰海通全天候指数 (CI011001.WI)
# ============================================================================

# CI011001 资产池：权益+债券+商品+现金，不含 nonferr（替换为铜期货更纯粹）
CI011001_ASSETS = [
    "hs300", "us_sp500", "credit",
    "bond_10y", "bond_30y",
    "gold", "copper", "wti",
...
# CI011001 期货杠杆因子 — 债券加杠杆提升风险贡献
# bond_10y: T.CFFEX 国债期货（2-3x 名义敞口），低波动债券核心杠杆工具
# bond_30y: 不额外加杠杆（数据层已有 3x 久期放大）
# copper/wti: 商品期货自带杠杆（保证金交易），通过 target_vol 控制总暴露
CI011001_LEVERAGE = {
    "bond_10y": 2.5,     # 国债期货 2.5x 名义杠杆
    "bond_30y": 1.0,     # 不再额外加杠杆
    "copper": 1.0,       # 期货自带杠杆，权重1.0表示标准保证金
    "wti": 1.0,
}

# CI011001 动态杠杆参数 — 市场危机时降杠杆
# 监控 N 个风险资产跌破趋势线 → 债券杠杆从 2.5x 降至 1.5x
CI011001_DYN_LEV = {
    "window": 120,                    # SMA 回看窗口（交易日）
    "monitor_assets": ["hs300", "us_sp500", "copper", "wti"],
    "threshold": 2,                   # 跌破趋势线资产数 ≥2 触发降杠杆
    "normal": {"bond_10y": 2.5},
    "crisis": {"bond_10y": 1.5},
}

# CI011001 ERC 参数
CI011001_ERC_WINDOW = 60       # ERC 协方差估计窗口（交易日）
CI011001_MAX_W = 0.25           # 单资产权重上限
CI011001_MIN_W = 0.02           # 单资产权重下限

# CI011001 目标波动率（双向）
CI011001_TARGET_VOL = 0.05      # 年化 5%
CI011001_VOL_FLOOR = 0.025      # 低于此值加杠杆至 1.5x（最低波动保障）
CI011001_VOL_CEILING_SCALE = 1.0  # 超过目标时直接缩放（无额外上限）

# 现金比例
CI011001_CASH_RATIO = 0.05      # 5% 现金缓冲

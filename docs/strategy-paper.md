...
本文档完整阐述一套面向 A 股市场的全天候投资策略体系的设计过程。策略以桥水基金 All Weather 风险平价框架为资产配置理论基础，以斯波朗迪（Victor Sperandeo）《专业投机原理》的"保障资本 → 一致性获利 → 卓越回报"三层哲学为方法论基础，映射到 A 股可投 ETF 资产，通过 2005-2026 年（~21 年）历史回测验证，最终产出三条不同风险偏好的可落地策略。核心结论：通过宏观象限分散 + 趋势过滤风控 + 估值锚定抄底的组合设计，可以在 A 股市场实现年化 8.48%-9.68%、最大回撤 -6.98% 至 -8.61%、V3-B 风险平价零负收益年份，V3-B 保守增强和 V3c 各一年负收益的稳健回报。
...
| 黄金 (gold) | 518850 | 通胀↑ 避险 | 实际利率下行 + 极端风险对冲 |
| 有色金属 (nonferr) | 159980 | 通胀↑ 商品 | 经济过热/通胀上行受益 |

> **原油（南方原油 LOF 501018）**：目前申购暂停。在恢复购买后可作为通胀↑桶补充资产加入组合，含原油版本 CAGR 略低 ~0.2-0.3pp，但 MDD 改善 1-2pp。详情见 README 含原油对比。
...
其中 σ_i 为资产 i 过去 20 个交易日的波动率。逻辑：波动越大的资产权重越小，波动越小的权重越大。这是你能做到的最简单的风险意识配置方式——不需要桶、不需要相关性矩阵、不需要优化器。

**为什么是 20d 而不是 60d？** 原版用 60d（更稳但反应慢），2026-08 网格验证后改为 20d：短窗口让权重更快响应波动率下降，与月度调仓节奏同频；配合 max_w=0.25 抑制 credit 过度集中（60d 下 credit 曾占 41.6%），CAGR 9.29%→9.73%（+0.47pp）且 MDD 1:1 对称。历史"20d 太吵"的顾虑在 max_w 收紧后不再成立。

**参数：**
- max_w = 0.25（单资产不超过 25%，抑制低波资产一家独大）
- min_w = 0.03（单资产不低于 3%，确保小配比不会被舍入为零）

**风控组合：**
- nonferr 趋势过滤（75d SMA）：有色金属跌破 75 日均线 → 清仓转入 credit
- gold 趋势过滤（75d SMA）：黄金跌破 75 日均线 → 清仓转入 credit
- sp500 趋势过滤（75d SMA）：标普 500 跌破 75 日均线 → 清仓转入 credit
- HS300 AND 抄底：价格回撤 ≥ 25% AND PB < 30% 分位 AND 价格 > SMA120 → 权重 ×1.8

**结果（2005-2026，含原油 + 月末调仓）：** CAGR 10.16%，MDD -6.30%，Sharpe 1.63，零负收益年，7 资产（含 wti）。月末最后交易日调仓（REBAL_MODE=month_end，避开商品期货月中换月窗口，单此一项 +0.43pp）。
...
3. sp500 趋势过滤（75d SMA）— 美股中长期趋势转熊时清仓转信用债
4. hs300 趋势过滤（30d SMA）— A 股急跌时提供额外权益保护（2015 股灾 +1.39pp）
5. HS300 AND 抄底 — 和 V3c 一致

**为什么是四个趋势过滤，而 V3c 有两个？** 因为 V3-B RP 的桶结构让每层趋势过滤针
对独立的宏观暴露来源。每条趋势过滤都通过了独立的 80/20 消融检验，且有因果关系支撑：

| 过滤器 | 因果逻辑 | 验证事件 |
|--------|----------|----------|
| nonferr 75d | 有色金属强周期，趋势性下跌持续数月 | 2013 年商品暴跌 |
| gold 75d | 黄金虽是避险资产，但在 taper tantrum 等流动性冲击中也暴跌 | 2013 年 -28% |
| sp500 75d | 美股中期趋势转熊预示经济衰退概率上升 | 2008 GFC |
| hs300 30d | A 股急跌迅速，30d 短窗口快速响应，月频调仓下假信号有限 | 2015 年股灾 |

**结果（2005-2026，含原油 + 月末调仓）：** CAGR 9.39%，MDD -6.63%，Sharpe 1.55，7 资产 4 桶，零负收益年份。
...
和 V3c 相同的公式，但关键区别在于：
- 窗口 20d（更敏捷）
- max_w = 0.25（与 V3c 相同；2026-08 起 V3c 亦收紧至 0.25 抑制 credit 集中）
- 8 资产（保留 bond_10y）
...
**结果（2005-2026，含原油 + 月末调仓）：** CAGR 8.35%，MDD -5.28%，Sharpe 1.82，8 资产，零负收益年。

### 3.5 三条策略的对比与选择逻辑

| | V3c 多元 | V3-B RP | V3-B Con |
|---|---|---|---|
| CAGR | 10.16% | 9.39% | 8.35% |
| MDD | -6.30% | -6.63% | -5.28% |
| Sharpe | 1.63 | 1.55 | 1.82 |
| 资产数 | 7 | 7 | 8 |
| 权重逻辑 | 逆波动率 20d | 4 桶等权+逆波动率 20d | 逆波动率 20d |
| 趋势过滤 | 3 个 | 4 个 | 1 个 |
| 负收益年 | 0 | 0 | 0 |

> 三策略均含原油（WTI 75d 趋势过滤）并采用月末最后交易日调仓（REBAL_MODE=month_end）。
...
### 6.4 HS300 趋势过滤（V3c/Con）

**想法：** 和 sp500 一样加 SMA 趋势过滤到 V3c 或 B-Con。

**为什么对 V3c/Con 失败：** CAGR -0.31pp。A 股波动大、趋势信号噪音多，月度调仓下错失反弹的代价大于回避主跌段的收益。

**为什么 V3-B RP 采用了 hs300(30d)：** 2015 年股灾中 V3-B RP 的 hs300 趋势过滤提供了 +1.39pp 的保护，且 30d 短窗口在月频调仓下假信号可接受。桶结构中每层暴露有独立宏观来源，hs300 趋势过滤针对增长↑桶的 A 股权益暴露，与 gold/sp500 不重叠。
...
| sp500 趋势窗口 | 75d | V3-B RP / V3c |
| hs300 趋势窗口 | 30d | V3-B RP |
